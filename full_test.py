...
parser.add_argument("--model_name", help="name of model", default="trial_2")
parser.add_argument(
    "--display_iter",
...
parser.add_argument("--train_input", help="folder name of input data", default="AIA")
parser.add_argument(
    "--test_input",
    help="folder name of input test data with magnetogram",
    default="AIA",
)
parser.add_argument("--test_data", help="folder name of test data", default="STEREO")
parser.add_argument(
    "--train_output", help="folder name of training output data", default="HMI"
)
parser.add_argument(
...
def TUMF_VALUE(IMAGE, RSUN, SATURATION, THRESHOLD):
    ...
    IMAGE_SCALE = SCALE(
        IMAGE, RANGE_IN=[0.0, 255.0], RANGE_OUT=[-SATURATION, SATURATION]
    )
    ...
    for I in range(SIZE_X):
        for J in range(SIZE_Y):
            if (I - SIZE_X / 2) ** 2.0 + (J - SIZE_Y / 2) ** 2.0 < RSUN**2.0:
    ...
    FACT = (695500.0 / RSUN) * (695500.0 / RSUN) * 1000 * 1000 * 100 * 100
...
while ITER <= MAX_ITER:
    ...
    MODEL_NAME = (
        "./MODELS/" + TRIAL_NAME + "/" + MODE + "/" + MODE + "_ITER" + SITER + ".h5"
    )
    ...
    for I in range(len(IMAGE_LIST1)):
        ...
        FAKE = NET_G_GEN(IMG)
        FAKE = ((FAKE[0] + 1) / 2.0 * 255.0).clip(0, 255).astype("uint8")
