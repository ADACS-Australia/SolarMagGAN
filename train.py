...
parser.add_argument("--input", help="folder name of input data", default="AIA")
parser.add_argument(
    "--display_iter",
...
def BASIC_D(ISIZE, NC_IN, NC_OUT, MAX_LAYERS):
    ...
    if MAX_LAYERS == 0:
        ...
        L = DN_CONV(N_FEATURE, kernel_size=1, padding="same", activation="sigmoid")(
            INPUT
        )
...
TRAINING_UPDATES_D = Adam(lr=2e-4, beta_1=0.5).get_updates(
    NET_D.trainable_weights, [], LOSS_D
)
...
TRAINING_UPDATES_G = Adam(lr=2e-4, beta_1=0.5).get_updates(
    NET_G.trainable_weights, [], LOSS_G
)
