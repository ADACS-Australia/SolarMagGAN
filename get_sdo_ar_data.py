...
res_ic = Fido.search(a.Time(time_ar[0], time_ar[1]),
                     a.jsoc.Series('hmi.ic_45s'),
                     a.jsoc.Notify(email)
                     )
res_m = Fido.search(a.Time(time_ar[0], time_ar[1]),
                    a.jsoc.Series('hmi.m_45s'),
                    a.jsoc.Notify(email)
                    )
res_304 = Fido.search(a.Time(time_ar[0], time_ar[1]),
                      a.jsoc.Notify(email),
...
# download data
# wait before running this command, as request nets to be processed:
